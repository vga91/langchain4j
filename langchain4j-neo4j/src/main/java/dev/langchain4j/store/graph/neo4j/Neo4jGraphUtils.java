...
public class Neo4jGraphUtils {
    ...
    public static void addGraphDocuments(
        ...
        for (GraphDocument graphDoc : graphDocuments) {
            ...
            if (includeSource) {
                ...
                final Map<String, Object> metadata = new HashMap<>(Map.copyOf(source.metadata().toMap()));
    ...
    private static String getNodeImportQuery(
            boolean baseEntityLabel,
            boolean includeSource,
            Neo4jGraph graph) {
    ...
    private static String getRelImportQuery(boolean baseEntityLabel, Neo4jGraph graph) {
        if (baseEntityLabel) {
            return """
                ...
                // MERGE (source)-[rel:$(toString(row.type1) + '')]->(target)
